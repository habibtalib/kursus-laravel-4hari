...
@section('title', 'Tambah Pembayar — Sistem Zakat Kedah')
...
@section('content')
    ...
    <x-card title="Tambah Pembayar Baru">
        <form method="POST" action="{{ route('pembayar.store') }}">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="alamat" id="alamat" rows="3"
                              class="w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 @error('alamat') border-red-500 @enderror">{{ old('alamat') }}</textarea>
                    @error('alamat')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
