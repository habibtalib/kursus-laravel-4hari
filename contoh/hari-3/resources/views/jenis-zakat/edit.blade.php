...
@section('title', 'Kemaskini Jenis Zakat — Sistem Zakat Kedah')
...
@section('content')
    ...
    <x-card title="Kemaskini Jenis Zakat: {{ $jenisZakat->nama }}">
        <form method="POST" action="{{ route('jenis-zakat.update', $jenisZakat) }}">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="kadar" id="kadar" value="{{ old('kadar', $jenisZakat->kadar) }}"
                           step="0.0001" min="0"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="penerangan" id="penerangan" rows="3"
                              class="w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 @error('penerangan') border-red-500 @enderror">{{ old('penerangan', $jenisZakat->penerangan) }}</textarea>
                    @error('penerangan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_aktif" value="1"
                               {{ old('is_aktif', $jenisZakat->is_aktif) ? 'checked' : '' }}
