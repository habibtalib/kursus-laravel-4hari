...
@section('title', 'Tambah Pembayaran — Sistem Zakat Kedah')
...
@section('content')
    ...
    <x-card title="Tambah Pembayaran Baru">
        <form method="POST" action="{{ route('pembayaran.store') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="tarikh_bayar" id="tarikh_bayar" value="{{ old('tarikh_bayar', date('Y-m-d')) }}"
                ...
                {{-- No Resit --}}
                <div>
                    <label for="no_resit" class="block text-sm font-medium text-gray-700 mb-1">No. Resit</label>
                    <input type="text" name="no_resit" id="no_resit" value="{{ old('no_resit', $noResit) }}"
                           readonly
                           class="w-full rounded-lg border-gray-300 bg-gray-100 shadow-sm text-gray-600 @error('no_resit') border-red-500 @enderror">
                    @error('no_resit')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
